...
self.onmessage = async ({ data: request }) => {
  ...
  try {
    if (
      request.state.board.size !== 15 ||
      !["FREESTYLE", "RENJU"].includes(request.state.ruleSet) ||
      (isAnalysis && request.state.ruleSet !== "RENJU")
    ) {
      analysisErrorCode = "UNSUPPORTED_POSITION";
      throw new Error("Classical Rapfi Worker only supports 15x15 Freestyle or Renju");
    }
    ...
    module.sendCommand(
      `INFO rule ${request.state.ruleSet === "RENJU" ? 2 : 0}`
    );
    ...
    if (isAnalysis) {
      ...
      self.postMessage({
        ...
        result: {
          ...
          engineUsed: "RAPFI_CLASSICAL",
          assetId: "classical-210901",
